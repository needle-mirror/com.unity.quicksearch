...
namespace UnityEditor.Search.Providers
{
    static class AssetProvider
    {
                ...
                get
                {
                    if (!m_Object)
                    {
                        ...
                        else
                        {
                            ...
                            #else
                            m_Object = AssetDatabase.LoadMainAssetAtPath(AssetDatabase.GUIDToAssetPath(gid.assetGUID));
                            #endif
                        }
                    }
                    return m_Object;
        ...
        private static Type GetItemAssetType(in SearchItem item, in Type constrainedType)
        {
            ...
            #if !USE_SEARCH_MODULE
            if (info.flags.HasAny(SearchDocumentFlags.Nested) && typeof(Sprite) == constrainedType && typeof(Texture2D).IsAssignableFrom(info.type))
