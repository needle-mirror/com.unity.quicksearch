...
namespace UnityEditor.Search
{
    ...
    class UndoManager
    {
        ...
        private void Save(string text, TextEditor textEditor, out int distance)
        {
            ...
            if (textEditor != null)
            {
                Debug.Log($"Save [{m_UndoIndex}] <b>{text}</b>, editor={textEditor?.text}, cursor={textEditor?.cursorIndex ?? -1}, previous={m_CurrentText}, d={distance}\n\t{PrintStack()}");
            }
            else
            {
                Debug.Log($"Init [{m_UndoIndex}] <b>{text}</b>\n\t{string.Join("\n\t", SearchSettings.recentSearches)}");
            }
            #endif
        ...
        {
            var i = 0;
            return string.Join("\n\t", m_UndoStack.Where(tu => tu.text != null).Select(tu => $"Stack [{i++}] <b>{tu.text}</b>, c={tu.cursorPos}, s={tu.selectPos}"));
        }
        #endif
        ...
        public bool HandleEvent(in Event evt, out string searchText, out int cursorPos, out int selectPos)
        {
            ...
            {
                #if DEBUG_UNDO_REDO
                Debug.LogWarning($"Undo [{m_UndoIndex}] <b>{searchText}</b>\n\t{PrintStack()}");
                #endif
