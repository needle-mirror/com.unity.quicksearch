...
namespace UnityEditor.Search
{
    ...
    public class ObjectField : BaseField<Object>
    {
        ...
        public static float singleLineHeight
        {
            ...
            {
                #if !USE_SEARCH_MODULE
                return ObjectFieldGUI.EditorGUI.kSingleLineHeight;
                #else
        ...
        VisualElement visualInput
        {
            get
            {
                var type = typeof(BaseField<Object>);
                var propInfo = type.GetProperty("visualInput", BindingFlags.Instance | BindingFlags.NonPublic);
                return propInfo?.GetMethod.Invoke(this, null) as VisualElement;
            }
            set
            {
                var type = typeof(BaseField<Object>);
                var propInfo = type.GetProperty("visualInput", BindingFlags.Instance | BindingFlags.NonPublic);
                propInfo?.SetMethod.Invoke(this, new object[] {value});
            }
        }
        #endif
        ...
        private class ObjectFieldDisplay : VisualElement
        {
            ...
            private Object DNDValidateObject()
            {
                ...
                #else
                Object validatedObject = ObjectFieldGUI.EditorGUI.ValidateObjectFieldAssignment(references, m_ObjectField.objectType, null, ObjectFieldGUI.EditorGUI.ObjectFieldValidatorOptions.None);
                #endif
    ...
    static class ObjectFieldGUI
    {
        ...
        internal static class EditorGUI
        {
            ...
            internal delegate Object ObjectFieldValidator(Object[] references, Type objType, SerializedProperty property, ObjectFieldValidatorOptions options);

            public static float kObjectFieldMiniThumbnailWidth => GetConstant<float>(nameof(kObjectFieldMiniThumbnailWidth));
            public static float kObjectFieldMiniThumbnailHeight => GetConstant<float>(nameof(kObjectFieldMiniThumbnailHeight));
            public static float kSingleLineHeight => GetConstant<float>(nameof(kSingleLineHeight));

            public static bool showMixedValue
            {
                get => (bool)CallPropertyGetter(nameof(showMixedValue));
                set => CallPropertySetter(nameof(showMixedValue), value);
            }

            public static Texture2D transparentCheckerTexture => CallPropertyGetter(nameof(transparentCheckerTexture)) as Texture2D;

            public static GUIContent mixedValueContent => CallPropertyGetter(nameof(mixedValueContent)) as GUIContent;

            public static Object ValidateObjectFieldAssignment(Object[] references, Type objType, SerializedProperty property, ObjectFieldValidatorOptions options)
            {
                return CallMethod(nameof(ValidateObjectFieldAssignment), references, objType, property, ConvertObjectFieldValidatorOptions(options)) as Object;
            }
            ...
            {
                CallMethod(nameof(BeginHandleMixedValueContentColor));
            }

            public static void EndHandleMixedValueContentColor()
            {
                CallMethod(nameof(EndHandleMixedValueContentColor));
            }

            public static GameObject GetGameObjectFromObject(Object obj)
            {
                return CallMethod(nameof(GetGameObjectFromObject), obj) as GameObject;
            }

            public static bool CheckForCrossSceneReferencing(Object obj1, Object obj2)
            {
                return (bool)CallMethod(nameof(CheckForCrossSceneReferencing), obj1, obj2);
            }
            ...
            {
                return (GenericMenu)CallMethod(nameof(FillPropertyContextMenu), property, linkedProperty, menu);
            }

            #region Helpers
            static object CallMethod(string name, params object[] parameters)
            {
                var mi = GetMethodInfo(name);
                return mi.Invoke(null, parameters);
            }

            static object CallMethod(string name)
            {
                var mi = GetMethodInfo(name);
            ...
            static MethodInfo GetPropertyGetterMethodInfo(string name)
            {
                var propInfo = GetPropertyInfo(name);
                return propInfo.GetMethod;
            }

            static MethodInfo GetPropertySetterMethodInfo(string name)
            {
                var propInfo = GetPropertyInfo(name);
            ...
            static object CallPropertyGetter(string name)
            {
                var mi = GetPropertyGetterMethodInfo(name);
                return mi.Invoke(null, null);
            }

            static object CallPropertySetter(string name, object value)
            {
                var mi = GetPropertySetterMethodInfo(name);
            ...
            static T GetConstant<T>(string name)
            {
                var fi = GetFieldInfo(name);
                return (T)fi.GetRawConstantValue();
            }

            static object ConvertObjectFieldValidatorOptions(ObjectFieldValidatorOptions options)
            {
                var intValue = (int)options;
                var enumType = GetNestedType(nameof(ObjectFieldValidatorOptions));
                return Enum.ToObject(enumType, intValue);
            }

            static Type GetNestedType(string name)
            {
                var type = typeof(UnityEditor.EditorGUI);
